...
void SentencePieceVocab::reservoirSampling(std::vector<std::string>& sample,
                                           size_t& seenLines,
...
size_t SentencePieceVocab::reservoirSamplingAll(io::TemporaryFile& temp,
                                                const std::vector<std::string>& trainPaths,
                                                size_t maxLines,
                                                size_t maxBytes) {
  LOG(info,
      "[SentencePiece] Sampling at most {} lines from {}",
      maxLines,
...
size_t SentencePieceVocab::dumpAll(io::TemporaryFile& temp,
                                   const std::vector<std::string>& trainPaths,
                                   size_t maxBytes) {
...
SentencePieceVocab::SentencePieceVocab(Ptr<Options> options, size_t batchIndex)
    : options_(options),
...
const std::string& SentencePieceVocab::canonicalExtension() const {
  return suffixes_[0];
}
const std::vector<std::string>& SentencePieceVocab::suffixes() const {
  return suffixes_;
}

std::string SentencePieceVocab::suffix() {
  return suffixes_[0];
};

std::string SentencePieceVocab::type() const {
  return "SentencePieceVocab";
}

Word SentencePieceVocab::getEosId() const {
  return Word::fromWordIndex(spm_->eos_id());
}
Word SentencePieceVocab::getUnkId() const {
  return Word::fromWordIndex(spm_->unk_id());
}

void SentencePieceVocab::create(const std::string& vocabPath,
                                const std::vector<std::string>& trainPaths,
                                size_t maxSize) {
  ...
  if(maxSize == 0) {
    LOG(info,
        "[SentencePiece] Vocabulary size is undefined (set with --dim-vocabs ...) - setting to {}",
        defaultMaxSize);
  ...
  command << " --bos_id=-1 --eos_id=0 --unk_id=1"  // these should not be changed as they match
                                                   // Marian defaults
          << " --input=" << tempFileName << " --model_prefix=" << vocabPath
          << " --vocab_size=" << maxSize << " --max_sentence_length=" << maxBytes
          << " --input_sentence_size=" << seenLines << " "
          << options_->get<std::string>(
                 "sentencepiece-options");  // these are SentencePiece command line options
  ...
  ABORT_IF(
      remove((vocabPath + ".vocab").c_str()) != 0, "Could not remove {}", vocabPath + ".vocab");
...
void SentencePieceVocab::createFake() {
...
std::string SentencePieceVocab::surfaceForm(const Words& sentence) const {
...
size_t SentencePieceVocab::load(const std::string& vocabPath, size_t /*maxSize*/) {
  LOG(info, "[data] Loading SentencePiece vocabulary from file {}", vocabPath);

  ABORT_IF(
      !filesystem::exists(vocabPath), "SentencePiece vocabulary file {} does not exist", vocabPath);
...
std::string SentencePieceVocab::toUpper(const std::string& line) const {
  return utils::utf8ToUpper(line);
}
std::string SentencePieceVocab::toEnglishTitleCase(const std::string& line) const {
  return utils::toEnglishTitleCase(line);
};
#endif  // USE_SENTENCEPIECE

Ptr<IVocab> createSentencePieceVocab(const std::string& vocabPath,
                                     Ptr<Options> options,
                                     size_t batchIndex) {
  ...
  if(isSentencePiece) {
    ...
    options;
    ABORT(
        "*.spm suffix in path {} reserved for SentencePiece models, "
